...
for (const snapshots of [false, true]) {
  test(`${snapshots ? "snapshot" : "observable"} runtime updates consumers without render-phase notifications`, async () => {
    ...
    try {
      ...
      expect(renders).toEqual({ active: snapshots ? 3 : 0, step: 3 });
      ...
      expect(renders).toEqual({ active: snapshots ? 4 : 1, step: snapshots ? 4 : 3 });
