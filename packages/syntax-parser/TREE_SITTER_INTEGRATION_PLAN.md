...
- [x] Diff consumer migration: `HybridDiffDocument` and merge previews use Tree-sitter with on-demand row queries, resumable full parsing, and corrected first-screen previews. See `../diff-parser/TREE_SITTER_BENCHMARKS.md`.
- [ ] Remaining shared consumer migration: replace TextMate state-stack tokenization in `HybridSyntaxDocument` and other legacy syntax-package callers before removing the old grammar engine.
