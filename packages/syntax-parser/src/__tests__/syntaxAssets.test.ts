...
describe("syntaxAssets", () => {
  it("detects syntax languages from common file paths", () => {
    ...
    expect(syntaxAssets.getSyntaxLanguageForPath("src/App.m")).toBe("objc");
    expect(syntaxAssets.getSyntaxLanguageForPath("src/App.mm")).toBe("");
  ...
  it("parses theme files defensively", () => {
    const { syntaxAssets } = loadSyntaxAssets();
  ...
  it("treats bundled themes as installed", () => {
    const { syntaxAssets } = loadSyntaxAssets();
