...
function calculateItemsInView(ctx, params = {}) {
  ...
  batchedUpdates(() => {
    ...
    const checkMVCP = doMVCP && !suppressInitialScrollSideEffects ? prepareMVCP(ctx, didDataChange, mvcpAdjustmentSource) : void 0;
...
function applyItemSize(ctx, itemKey, sizeObj, resolvedMeasurementItem) {
  ...
  const applyMVCPAdjustment = state.props.maintainVisibleContentPosition.size ? prepareMVCP(ctx, false, "item-size") : void 0;
...
function resolveFixedItemSize(ctx, containerId, itemKey) {
  ...
  const index = state.indexByKey.get(itemKey);
  let fixedItemSize;
  if (getFixedItemSize && index !== void 0) {
...
function processContainerLayout({ containerId, ctx, rectangle, ref, state }) {
  ...
  if (shouldDeferWebShrinkLayoutUpdate) {
    ...
    if (IsNewArchitecture || size > 0) {
      ...
      (_c = (_b = ref.current) == null ? void 0 : _b.measure) == null ? void 0 : _c.call(_b, (_x, _y, width, height) => {
        layout = { height, width };
        applyLayout();
      });
    }
...
var LegendListInner = typedForwardRef(function LegendListInner2(props, forwardedRef) {
  ...
  const [, scheduleDataSourceCommit] = React2.useReducer((value) => value + 1, 0);
  const ctx = useStateContext();
