...
if (!output) {
  throw new Error("Usage: bun run hello-world:benchmark:export --output <absolute .app path>");
}
...
if (!process.argv.includes("--skip-build")) {
  runCommand("bun", ["scripts/build-app.ts", "hello-world", "macos", arch], {
    cwd: rootDir,
  });
}

const manifest = await loadAppManifest("hello-world");
