...
function encode(value: string): Uint8Array {
    ...
    return new Uint8Array(Array.from(unescape(encodeURIComponent(value))).map((character) => character.charCodeAt(0)));
}

function sha256Fallback(value: string): ArrayBuffer {
    ...
    view.setUint32(buffer.length - 4, (bytes.length * 8) >>> 0, false);
