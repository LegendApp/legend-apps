...
export function TypeGPU({
  ...
  useEffect(() => {
    ...
    return startTypeGPUCanvas({
      canvasRef, height, playback, resume, scene, setError, width,
      report: (caught) => reportSlideError(caught, slideIndex, playback.current.isPreview),
    });
...
// GPU setup and frame callbacks own their mutable resources outside React rendering.
// Keeping this async lifecycle separate also avoids Compiler 1.0's try/catch limitations.
